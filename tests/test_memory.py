...
class TestMemoryRecord:
    def test_default_creation(self):
        ...
        assert record.record_type == MemoryType.OBSERVATION
        assert record.lifecycle == RecordLifecycle.NEW

    def test_with_metadata(self):
        record = MemoryRecord(
            id="r2", content="hello", cycle=5,
            importance=0.9, record_type=MemoryType.EPISODE,
            metadata={"source": "camera"},
        )
        assert record.importance == 0.9
        assert record.record_type == MemoryType.EPISODE
...
class TestMemoryQuery:
    ...
    def test_filtered_query(self):
        ...
        assert q.memory_types == [MemoryType.EPISODE]
...
class TestFIFOMemoryStrategy:
    def test_store_and_retrieve(self):
        ...
        result = s.retrieve(MemoryQuery(max_results=10))
    ...
    def test_fifo_eviction(self):
        ...
        assert s.size == 3
        ids = {r.id for r in s.retrieve(MemoryQuery(max_results=10)).records}
...
class TestMemory:
    ...
    def test_different_strategies_same_interface(self):
        ...
        for strategy in strategies:
            ...
            result = strategy.retrieve(MemoryQuery(max_results=10))
...
class TestMemoryIntegration:
    def test_memory_stores_observations_across_cycles(self):
        ...
        result = mem.ask(MemoryQuery(memory_types=[MemoryType.OBSERVATION], max_results=10))
